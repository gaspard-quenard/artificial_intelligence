...
def expected_return(state, action, value_function):
    ...
    exp_return =  0 - (new_state[0] >= 10) * 4  - (new_state[1] >= 10) * 4 # We lose 4 dollar if we have more than 10 car in a location after having moving the cars
    if (action != 0):
        exp_return = exp_return -2 * (abs(action) - 1) # Because we can move one car to the second location for free
...
if __name__ == '__main__':
    ...
    while True:
        ...
        print("Press q to exit the display")
